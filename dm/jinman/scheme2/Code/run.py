...
def feature_engineer_duration(data):#时差
    raw = data.copy()
    df = pd.DataFrame(raw['样本id'])
    # 加热过程
...
def make_train(train, test, *train_add):
    ...
    cols_timer = [
        'A5', 'A7', 'A9', 'A11', 'A14', 'A16', 'A24', 'A26', 'B5', 'B7']#时间变量
    ...
    for _df in [df_trn_tst, df_trn, df_tst]:
        for _col in cols_timer:
            _df[_col] = _df[_col].map(time_to_min)#时间转为分钟？
...
def make_prediction(train, test, sed):
    ...
    for i in tqdm.tqdm(range(len(df_seed))):
        ...
        for _f in tst.columns.tolist()[1:-1]:#142
            vc = trn[_f].value_counts(normalize=True)
            corr = np.abs(np.corrcoef(trn[_f], trn['收率']))[0, 1]#相关系数
            if len(vc) <= 18 and corr >= 0.36 and vc.iloc[0] <= 0.75:
                for _col in [f'recall_{i}' for i in range(1, 5)]:
                    ...
                    for _df in [trn, tst]:
                        ...
                        _df.insert(_idx_col + 1, f'{_f}_{_col}_count',_df[_f].map(recall_count))
...
def main():
    ...
    df_trn = train_abnormal_revise(df_trn).pipe(train_abnormal_adjust)#处理异常值
