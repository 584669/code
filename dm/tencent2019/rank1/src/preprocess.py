...
def construct_log():
    ...
    #'id','request_timestamp','position','uid','aid','imp_ad_size','bid','pctr','quality_ecpm','totalEcpm'
    #广告请求 id，广告请求时间：该字段为时间戳，即 1970 纪元后经过的浮点秒数request_timestamp，
    #广告位 position，用户 uid，曝光广告 aid，曝光广告素材尺寸imp_ad_size，曝光广告出价 bid，曝光广告 pctr，曝光广告 quality_ecpm，曝光广告 totalEcpm
    train_df=pd.read_pickle('data/testA/totalExposureLog.pkl')
    print(train_df.head(5))
    #处理曝光时间
    train_df['request_day']=train_df['request_timestamp']//(3600*24)#第多少天
    wday=[]
    hour=[]
    minute=[]
    ...
    #
    train_df['period_id']=train_df['hour']*2+train_df['minute']//30#一天的具体时间分成48个时区
    dev_df=train_df[train_df['request_day']==17974]#第17974天什么意思？
    del dev_df['period_id']
    del dev_df['minute']
    del dev_df['hour']
    log=train_df
    tmp = pd.DataFrame(train_df.groupby(['aid','request_day']).size()).reset_index()
    tmp.columns=['aid','request_day','imp']#imp曝光次数？
    print(tmp.head(5))
    log=log.merge(tmp,on=['aid','request_day'],how='left')#sql左链接
    print(log.head(5))
    #分测试验证
